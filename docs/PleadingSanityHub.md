...
3. Functions call OpenAI completion API (Chat Completions) via `routeMessage`; replies stored as `type: 'ai'` in Firestore plus mirrored context.
4. Additional AIs (Copilot/Meta) can be wired by toggling `enabled` and replacing handler implementations.

## Deployment

- GitHub Actions uses `cosmic-deploy.yml` (Node 20, Netlify CLI) to deploy `public` + functions.
- Local preview: `npm install`, `netlify dev` (needs Netlify CLI globally or add devDependency).
- Vercel alternative: move functions to `api/` and configure rewrites (notes inline in docs).
